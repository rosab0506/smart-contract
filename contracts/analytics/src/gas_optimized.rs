...
use shared::gas_optimizer::{
    extend_instance_if_needed, unpack_u32, pack_u32, BatchResult,
    TTL_BUMP_THRESHOLD,
};
...
impl PackedMetrics {
    pub fn active_learners(&self) -> u32 { unpack_u32(self.learners_and_score).0 }
    pub fn avg_score_pct(&self) -> u32   { unpack_u32(self.learners_and_score).1 }
...
pub fn record_event_optimized(env: &Env, learner: &Address, event_type: u32, value: u64) {
    ...
    match event_type {
        ...
        2 => {
            ...
            let new_avg = if learners == 0 { value as u32 }
                else { (((prev_avg as u64) * (learners as u64) + value) / (learners as u64 + 1)) as u32 };
    ...
    env.storage().temporary().set(&ev_key, &CompactEvent {
        learner: learner.clone(), event_type, value, ledger_seq: env.ledger().sequence(),
    });
...
pub fn batch_record_events(env: &Env, _learner: &Address, events: &Vec<(u32, u64)>) -> BatchResult {
    ...
    for i in 0..events.len() {
        if let Some((event_type, value)) = events.get(i) {
            match event_type {
                0 => { metrics.total_views += 1; result.processed += 1; }
                1 => {
                    metrics.total_completions += 1;
                    metrics.total_time_secs = metrics.total_time_secs.saturating_add(value);
                    result.processed += 1;
