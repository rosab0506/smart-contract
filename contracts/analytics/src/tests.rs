...
mod analytics_tests {
    ...
    fn create_test_session(env: &Env, student: &Address, course_id: &str, module_id: &str) -> LearningSession {
    ...
    fn test_get_struggling_students() {
        ...
        for (student, completion) in [(&student1, 100), (&student2, 30)] {
            ...
            let end_time = session.start_time + 1800;
            client.complete_session(&session.session_id, &end_time, &Some(completion), &completion);
        }
