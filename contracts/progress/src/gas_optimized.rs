...
use shared::gas_optimizer::{BatchResult, TTL_PERSISTENT_YEAR, TTL_BUMP_THRESHOLD};
...
impl PackedProgress {
    pub fn is_module_complete(&self, idx: u8) -> bool { (self.module_flags >> idx) & 1 == 1 }
    ...
    pub fn completed_module_count(&self) -> u32 { self.module_flags.count_ones() }
    pub fn score_x10(&self) -> u16     { (self.score_and_meta >> 48) as u16 }
    pub fn completion_pct(&self) -> u8 { ((self.score_and_meta >> 40) & 0xFF) as u8 }
    pub fn started_ledger(&self) -> u32 { (self.score_and_meta & 0xFFFF_FFFF) as u32 }
...
pub fn start_course_optimized(env: &Env, learner: &Address, course_id: u32) {
    ...
    if env.storage().persistent().has(&progress_key(learner, course_id)) { return; }
    let mut prog = PackedProgress::default();
    prog.set_started_ledger(env.ledger().sequence());
    save_progress(env, learner, course_id, &prog);
}

pub fn complete_module_optimized(env: &Env, learner: &Address, course_id: u32, module_idx: u8, total_modules: u8) -> bool {
    learner.require_auth();
    let mut prog = load_progress(env, learner, course_id);
    if !prog.mark_module_complete(module_idx) { return false; }
...
pub fn batch_complete_modules(env: &Env, learner: &Address, course_id: u32, module_indices: &Vec<u32>, total_modules: u8) -> BatchResult {
    learner.require_auth();
    let mut prog = load_progress(env, learner, course_id);
    ...
    for i in 0..module_indices.len() {
        if let Some(idx) = module_indices.get(i) {
            ...
            if prog.mark_module_complete(idx as u8) { result.processed += 1; }
