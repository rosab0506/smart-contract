...
use soroban_sdk::{contract, contractimpl, contracterror, symbol_short, Address, Env, Symbol, Vec, Map};
...
impl Progress {
    ...
    pub fn get_completion_percentage(env: Env, user: Address, course_id: Symbol) -> Result<u32, Error> {
        // Get course progress
...
mod test {
    use super::*;
    use soroban_sdk::testutils::{Address as _, Symbol as _};
    
    #[test]
    fn test_initialize() {
        let env = Env::default();
        let contract_id = env.register_contract(None, Progress);
    ...
    fn test_course_management() {
        ...
        client.add_course(&course_id, &total_modules).unwrap();
        
        // Get course modules
        let modules = client.get_course_modules(&course_id).unwrap();
        assert_eq!(modules, total_modules);
        
        // Try to get non-existent course
        let invalid_course = Symbol::short("INVALID");
        let result = client.get_course_modules(&invalid_course);
        assert_eq!(result, Err(Error::CourseNotFound));
    }
    ...
    fn test_user_progress() {
        ...
        client.update_progress(&user, &course_id, &1, &true).unwrap();
        
        // Get progress
        let progress = client.get_progress(&user, &course_id).unwrap();
        assert_eq!(progress.len(), total_modules as u32 + 1); // +1 because modules are 1-indexed
        assert_eq!(progress.get(1), Some(true));
        
        // Update progress for module 2
        client.update_progress(&user, &course_id, &2, &true).unwrap();
        
        // Get completion percentage
        let percentage = client.get_completion_percentage(&user, &course_id).unwrap();
        assert_eq!(percentage, 20); // 2 out of 10 modules = 20%
        
        // Try to update invalid module
        let result = client.update_progress(&user, &course_id, &0, &true);
        assert_eq!(result, Err(Error::InvalidProgress));
        
        let result = client.update_progress(&user, &course_id, &11, &true);
        assert_eq!(result, Err(Error::InvalidProgress));
    }
}
